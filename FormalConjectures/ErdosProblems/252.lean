...
theorem erdos_252 :
    answer(sorry) ↔ ∀ k ≥ 1, Irrational (∑' n, σ k n / (n ! : ℝ)) := by
  sorry

/-- `∑ σ 0 n / n!` is irrational. This is proved in [ErSt71]. -/
@[category research solved, AMS 11]
theorem erdos_252.variants.k_eq_zero : Irrational (∑' n, σ 0 n / (n ! : ℝ)) := by
  sorry

/-- `∑ σ 1 n / n!` is irrational. This is proved in [ErSt74]. -/
@[category research solved, AMS 11]
theorem erdos_252.variants.k_eq_one : Irrational (∑' n, σ 1 n / (n ! : ℝ)) := by
  sorry


/-- `∑ σ 2 n / n!` is irrational. This is proved in [ErKa54]. -/
@[category research solved, AMS 11]
theorem erdos_252.variants.k_eq_two : Irrational (∑' n, σ 2 n / (n ! : ℝ)) := by
  sorry

/-- `∑ σ 3 n / n!` is irrational. This is proved in [ScPu06] and [FLC07]. -/
@[category research solved, AMS 11]
theorem erdos_252.variants.k_eq_three : Irrational (∑' n, σ 3 n / (n ! : ℝ)) := by
  sorry

/-- `∑ σ 4 n / n!` is irrational. This is proved in [Pr22]. -/
@[category research solved, AMS 11]
theorem erdos_252.variants.k_eq_four : Irrational (∑' n, σ 4 n / (n ! : ℝ)) := by
  sorry

/-- For a fixed `k ≥ 5`, is `∑ σ k n / n!` irrational?. -/
@[category research open, AMS 11]
theorem erdos_252.variants.k_ge_five :
    answer(sorry) ↔ ∀ k ≥ 5, Irrational (∑' n, σ k n / (n ! : ℝ)) := by
  sorry
